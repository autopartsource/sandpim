...
{
 $logs->logSystemEvent('autocareupdate', 0, 'Qdb import skipped (host,user,password or downloads directory are not configured)');
 exit;
}
...
exec('wget --quiet --ftp-user='.$username.' --ftp-password='.$password.' --no-check-certificate ftps://'.$host.'/download_qdb/MySQL/AAIA%20Qdb%20MySQL%20'.$dbversion.'.zip --output-document='.$downloadsdirectory.'/'.$randomfilename);
echo "Done\n";
...
if($archivesize>0)
{
 ...
 if(file_exists($downloadsdirectory.'/'.'AAIA Qdb MySQL '.$dbversion.'.sql'))
 {
  // create the new database
  $dbcreationresult=$pim->createAutoCareDatabase('qdb'.$dbversion, $mysql->user);
  if($dbcreationresult=='success')
  {
   ...
   $logs->logSystemEvent('autocareupdate', 0, 'qdb '.$dbversion.' imported');
  }
  else
  {// database create failed
   echo 'database create failed: '.$dbcreationresult."\n";
   $logs->logSystemEvent('autocareupdate', 0, 'qdb import failed ('.$dbcreationresult.')');
  }
 
  exec('rm -f '.$downloadsdirectory.'/'.'AAIA\ Qdb\ MySQL\ '.$dbversion.'.sql');

 }
 else
 {
  echo "did not find the epected SQL in the downloaded zip file\n";    
  $logs->logSystemEvent('autocareupdate', 0, 'Qdb import failed - archive did not contain expected .sql filename');
 }

}
else
{// downloaded zipfile filesiize is 0
 $logs->logSystemEvent('autocareupdate', 0, 'Qdb import failed - filesize 0');
}
