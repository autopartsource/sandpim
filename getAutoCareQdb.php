...
{//2020-10-30
 ...
 if(count($qdbsavailable)>0)
 {
  foreach($qdbsavailable as $qdbavailable)
  {
   if($_GET['versiondate']==$qdbavailable['versiondate'])
   {
    $found=true;
 ...
 if($archivesize>0)
 {
  ...
  if(file_exists($downloadsdirectory.'/'.'AAIA Qdb MySQL '.$dbversion.'.sql'))
  {
   ...
   {// database create failed
    ...
    $logs->logSystemEvent('autocareupdate', $_SESSION['userid'], 'qdb import failed ('.$dbcreationresult.')');
   }
  ...
  else
  {
   ...
   $logs->logSystemEvent('autocareupdate', $_SESSION['userid'], 'Qdb import failed - archive did not contain expected .sql filename');
  }
 }
 else
 {// downloaded zipfile filesiize is 0
  $logs->logSystemEvent('autocareupdate', $_SESSION['userid'], 'Qdb import failed - filesize 0');
 }
