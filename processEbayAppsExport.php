...
$pim = new pim();
$jobs=$pim->getBackgroundjobs('EbayExport','started');
...
//     $output[]=$app['partnumber']."\t".$ebayvehicle['makename']."\t".$ebayvehicle['modelname']."\t".$ebayvehicle['year']."\t".$niceengine."\t".$nicesubmodelname.' '.$ebayvehicle['bodytypename'].' '.$ebayvehicle['bodynumdoors'].'-Door'."\t".$nicefitmentstring."\r\n";
     $writeresult=fwrite($fh, $app['partnumber']."\t".$ebayvehicle['makename']."\t".$ebayvehicle['modelname']."\t".$ebayvehicle['year']."\t".$niceengine."\t".$nicesubmodelname.' '.$ebayvehicle['bodytypename'].' '.$ebayvehicle['bodynumdoors'].'-Door'."\t".$nicefitmentstring."\r\n");
    }
   }
  }
  ...
  $pim->logBackgroundjobEvent($jobid, 'Ebay applications file ['.$filename.'] created containing '.count($apps).' apps for profile id '.$receiverprofileid);
  $logs->logSystemEvent('Export', 0, 'Ebay applications file ['.$filename.'] (jobid:'.$jobid.') exported by houskeeper; apps: '.count($apps));
 }
 else
 { // open failed
  $pim->updateBackgroundjobDone($jobid,'failed',date('Y-m-d H:i:s'));
  $pim->logBackgroundjobEvent($jobid, 'file write failed ['.$filename.']' );
  $logs->logSystemEvent('Export', 0, 'Ebay applications file ['.$filename.'] (jobid:'.$jobid.') export failed (write permission denied?) during houskeeper processing; apps: '.count($apps));     
 } 
}
else
{
