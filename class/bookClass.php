...
class book
{
  ...
  foreach($data as $makename=>$models)
  {
   foreach($models as $modelname=>$apps)
   {
    foreach($apps as $app)
    {
     ...
     if(array_key_exists($makename, $compressed))
     {
      ...
      {// make/model already exist - roll through existing rows to find a compatibles one to glom onto 
       ...
       {// found existing row that is year-contiguous with the row we are trying to add
        $compressed[$makename][$modelname][$existingrow]['qualifierblocks'][$app['qualifiers']][]=$app['partnumber'];
        $appwasplaced=true;
       } 
      }   
     }
     ...
     if(!$appwasplaced)
     {
      $compressed[$makename][$modelname][]=array('year'=>$app['year'],'qualifierblocks'=>array($app['qualifiers']=>array($app['partnumber'])));
     }
    }
   }
  }
    ...
    foreach($rows as $row)
    {
     ...
     if(array_key_exists($makename, $output))
     {
      ...
      {// make/model already exist - roll through existing rows to find a compatibles one to glom onto 
       ...
       $existingrow=$this->existingXrow($row, $output[$makename][$modelname]);
                                        
       if($existingrow>=0)
       {
        
        $output[$makename][$modelname][$existingrow]['endyear']=$row['year'];
        $rowwasplaced=true;
       }   
      }
     }
 ...
 function getContentOld($partcategories)
 {
  $categoryarray=array(); foreach($partcategories as $partcategory){$categoryarray[]=intval($partcategory);} $categorylist=implode(',',$categoryarray); // sanitize input
  $db = new mysql; 
...
function existingXrow($needle,$haystack)
{
 ...
 foreach($haystack as $i=>$element)
 {
  ...
  if(($blockshash==$needleblockhash) && $needle['year']==($element['endyear'])+1)
  {
   $existingrow=$i; break;
  }
 }
