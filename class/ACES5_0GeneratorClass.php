...
class ACESgenerator
{
 ...
 // ---------------------- header ------------------------------
  ...
  if(array_key_exists('BrandAAIAID', $header)){$brandAAIAIDElement=new DOMElement('BrandID',$header['BrandAAIAID']); $headerElement->appendChild($brandAAIAIDElement);}
  if(array_key_exists('SubBrandAAIAID', $header)){$brandAAIAIDElement=new DOMElement('SubBrandID',$header['BrandAAIAID']); $headerElement->appendChild($brandAAIAIDElement);}
  ...
  if(array_key_exists('PartsApprovedFor', $header))
  { // country codes are jammed in as a coma-delimited list
   $partsapprovedfors=explode(',',$header['PartsApprovedFor']);            
   $partsApprovedForElement=new DOMElement('PartsApprovedFor');
   $headerElement->appendChild($partsApprovedForElement);
   foreach($partsapprovedfors as $partsapprovedfor)
   {
    $countryElement=new DOMElement('Country',$partsapprovedfor);
    $partsApprovedForElement->appendChild($countryElement);
   }
  }

  if(array_key_exists('RegionFor', $header))
  { // regions are jammed in as a coma-delimited list
   $regionfors=explode(',',$header['RegionFor']);            
   $regionForElement=new DOMElement('RegionFor');
   $headerElement->appendChild($regionForElement);
   foreach($regionfors as $regionfor)
   {
    $regionElement=new DOMElement('Region');
    $regionForElement->appendChild($regionElement);
    $regionElement->setAttribute('id', $regionfor);    
   }   
  }
  ...
  if(array_key_exists('VcdbPublicationDate', $header)){$vcdbpublicationdateElement=new DOMElement('VcdbPublicationDate',$header['VcdbPublicationDate']); $headerElement->appendChild($vcdbpublicationdateElement);}
  if(array_key_exists('QdbPublicationDate', $header)){$qdbpublicationdateElement=new DOMElement('QdbPublicationDate',$header['QdbPublicationDate']); $headerElement->appendChild($qdbpublicationdateElement);}
  if(array_key_exists('PcdbPublicationDate', $header)){$pcdbpublicationdateElement=new DOMElement('PcdbPublicationDate',$header['PcdbPublicationDate']); $headerElement->appendChild($pcdbpublicationdateElement);}
  ...
  foreach($apps as $app)
  {
   ...
   $parttypeElement=new DOMElement('PartTerminology');
   ...
   {// traanslation exist for this app's partnumber - flip it
    $partElement=new DOMElement('PartNumber',$parttranslations[$app['partnumber']]);    
   }
   else
   {// no traanslation exist for this app's partnumber 
    $partElement=new DOMElement('PartNumber',$app['partnumber']);
   }

   $appElement->appendChild($partElement);
   if(array_key_exists('brand',$app) && $app['brand']!=''){$partElement->setAttribute('BrandID', $app['brand']);}
   if(array_key_exists('subbrand',$app) && $app['subbrand']!=''){$partElement->setAttribute('SubBrandID', $app['subbrand']);}
