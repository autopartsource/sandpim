...
{   
 function createPIESdoc($header,$marketingcopys,$items,$generatoroptions=false)
 {
  ...
  $PIESVersionElement=new DOMElement('PIESVersion','8.0');
...
//  if(array_key_exists('BrandOwnerAAIAID', $header)){$BrandOwnerIDelement=new DOMElement('BrandOwnerID',$header['BrandOwnerAAIAID']); $headerElement->appendChild($BrandOwnerIDelement);}   
  ...
  if(array_key_exists('PCdbVersionDate',$header)){$PCdbVersionDateElement=new DOMElement('PCdbPublicationDate',$header['PCdbVersionDate']);  $headerElement->appendChild($PCdbVersionDateElement);}
  if(array_key_exists('PAdbVersionDate',$header)){$PAdbVersionDateElement=new DOMElement('PAdbPublicationDate',$header['PAdbVersionDate']);  $headerElement->appendChild($PAdbVersionDateElement);}
  ...
  foreach($items as $PartNumber=>$item)
  {
   ...
   if(isset($item['assets']) && count($item['assets']))
   {
    ...
    $DigitalAssetsElement->setAttribute('FileHashType', 'MD5');
    ...
    foreach($item['assets'] as $asset)
    {
     ...
     if(array_key_exists('FileHash',$asset) && strlen($asset['FileHash'])==32){$FileHashElement=$doc->createElement('FileHash',$asset['FileHash']); $DigitalFileInformationElement->appendChild($FileHashElement);}
