...
if(isset($_POST['submit']) && $_POST['submit']=='Generate Excel file')
{
 if($_FILES['fileToUpload']['type']=='text/xml' || $_FILES['fileToUpload']['type']=='')
 {
  ...
  {     
   ...
   if($validatexsd && !$doc->schemaValidate('PIES_8_0_XSDSchema_Rev3_10_2_2025.xsd'))
   {
...
if($validUpload)
{
 ...
 if(count($headerElement))
 {
  ...
  $parentIDelement=$headerElement[0]->getElementsByTagName('ParentID');
  if(count($parentIDelement)){$header['ParentID']=$parentIDelement[0]->nodeValue;}
  ...
  $brandownerIDelement=$headerElement[0]->getElementsByTagName('BrandOwnerID');
  if(count($brandownerIDelement)){$header['BrandOwnerID']=$brandownerIDelement[0]->nodeValue;}
 ...
 foreach ($itemElements AS $itemElement) 
 {
  ...
  $brandid=$itemElement->getElementsByTagName('BrandID')[0]->nodeValue;
 ...
 */
  ...
  if(count($kitsElement))
  {
   ...
   {
    $componentpartnumber=''; $description=''; $descriptioncode=''; $quantityinkit=''; $quantityinkituom=''; $soldseparately=''; $languagecode=''; $componentpartterminologyid=''; $sequencecode=''; $componentbrand=''; $componentsubbrand='';
  ...
  if(count($partinterchangeinfoElement))
  {
   ...
   foreach($partinterchangeElements as $partinterchangeElement)
   {
    ...
    $interchangebrandID=$partinterchangeElement->getAttribute('BrandID');
    $subbrandID=$partinterchangeElement->getAttribute('SubBrandID');
    ...
    {  // possible multiple parts per competitor
     ...
     $interchanges[]=array('CompetitorPartNumber'=>$competitorpartnumber,'ReferenceItem'=>$referenceitem,'InterchangeQuantity'=>$interchangequantity,'UOM'=>$interchangequantityuom,'InterchangeNotes'=>$interchangenotes,'BrandID'=>$interchangebrandID,'SubBrandID'=>$subbrandID,'InternalNotes'=>$internalnotes,'LanguageCode'=>$languagecode,'VMRSBrandID'=>$VMRSbrandid, 'QualityGradeLevel'=>$qualitygradelevel, 'ItemEquivalentUOM'=>$itemequivalentuom);
    }
   }
  }
  ...
  if(count($digitalassetsElement))
  {
   ...
   {
    $filename=''; $assettype=''; $filetype=''; $representation=''; $background=''; $orientationview=''; $uri=''; $country=''; $filesize=''; $filehash=''; $resolution=''; $colormode=''; $filepath=''; $frame=''; $totalframes=''; $plane=''; $hemisphere=''; $plunge=''; $totalplanes='';
    ...
    $filehashElement=$digitalfileinformationElement->getElementsByTagName('FileHash');
    if(count($filehashElement)){$filehash=$filehashElement[0]->nodeValue;}
 ...
 $row=array('BrandOwnerID',$header['BrandOwnerID']); $writer->writeSheetRow('Header', $row);
 ...
 //---------------- items -----------------
 $writer->writeSheetHeader('Items', array(
     'PartNumber'=>'string',
     ...
     'MinimumOrderQuantityUOM'=>'string',
     'HazardousMaterialCode'=>'string',
     'BaseItemID'=>'string',
     'ItemEffectiveDate'=>'string',
     'AvailableDate'=>'string',
     'ACESApplications'=>'string',
     'ItemQuantitySize'=>'integer',
     'ItemQuantitySizeUOM'=>'string',
     'ContainerType'=>'string',
     'QuantityPerApplication'=>'integer',
     'QuantityPerApplicationUOM'=>'string',
     'VMRSBrandID'=>'string',
     'UNSPC'=>'string'),        array('widths'=>array(
         12,
         16,
         12,
         29,
         ...
         24,
         13,
         9),'freeze_rows'=>1, 'freeze_columns'=>1,
         ['fill'=>'#ff0000'],
         ['fill'=>'#ff0000'],
         ['fill'=>'#ff0000'],
         ['fill'=>'#c0c0c0'],
 ...
 {
  $row=array($partnumber,$item['PartTerminologyID'],$item['BrandID'],$item['NicePartTerminologyName'],$item['ItemLevelGTIN'],$item['GTINQualifier'],$item['MinimumOrderQuantity'],$item['MinimumOrderQuantityUOM'],$item['HazardousMaterialCode'],$item['BaseItemID'],$item['ItemEffectiveDate'],$item['AvailableDate'],$item['ACESApplications'],$item['ItemQuantitySize'],$item['ItemQuantitySizeUOM'],$item['ContainerType'],$item['QuantityPerApplication'],$item['QuantityPerApplicationUOM'],$item['VMRSBrandID'],$item['UNSPSC']);
...
//------------- kits ----------------
 $writer->writeSheetHeader('Kits', array(
     'PartNumber'=>'string',
     'Description'=>'string',
     'DescriptionCode'=>'string',
     'QuantityInKit'=>'integer',
     'QuantityInKitUOM'=>'string',
     'SoldSeparately'=>'string',
     'LanguageCode'=>'string',
     'ComponentPartTerminologyID'=>'integer',
     'SequenceCode'=>'integer',
     'ComponentPartNumber'=>'string',
     'ComponentBrand'=>'string',
     'ComponentSubBrand'=>'string'), array('widths'=>array(
       11,
       41,
       15,
       12,
       ...
       21,
       16,
       19),'freeze_rows'=>1, 'freeze_columns'=>1,
       ['fill'=>'#ff0000'],
       ['fill'=>'#ff0000'],
       ['fill'=>'#ff0000'],
       ['fill'=>'#ff0000'],
 ...
 foreach($items as $partnumber=>$item)
 {
  foreach($item['kits'] as $kit)
  {
   $row=array(
       ...
       $kit['DescriptionCode'],
       $kit['QuantityInKit'],
       $kit['QuantityInKitUOM'],
       $kit['SoldSeparately'],
       $kit['LanguageCode'],
       $kit['ComponentPartTerminologyID'],
       $kit['SequenceCode'],
       $kit['ComponentPartNumber'],
       $kit['ComponentBrand'],
       $kit['ComponentSubBrand']);
   $writer->writeSheetRow('Kits', $row);
  }
 }
 
 //------------- interchnges ---------
 $writer->writeSheetHeader('Interchanges', array(
     'PartNumber'=>'string',
     'CompetitorPartNumber'=>'string',
     'BrandID'=>'string',
     'InterchangeQuantity'=>'integer',
     'UOM'=>'string',
     'ReferenceItem'=>'string',
     'InterchangeNotes'=>'string',
     'ItemEquivalentUOM'=>'string',
     'LanguageCode'=>'string',
     'SubBrandID'=>'string',
     'VMRSBrandID'=>'string',
     'QualityGradeLevel'=>'string',
     'InternalNotes'=>'string'), array('widths'=>array(
         12,
         21,
         12,
         18,
         ...
         14,
         17,
         12),'freeze_rows'=>1, 'freeze_columns'=>1,
         ['fill'=>'#ff0000'],
         ['fill'=>'#ff0000'],
         ['fill'=>'#ff0000'],
         ['fill'=>'#ffff00'],
 ...
 foreach($items as $partnumber=>$item)
 {
  foreach($item['interchanges'] as $interchange)
  {
   $row=array(
       ...
       $interchange['BrandID'],
       $interchange['InterchangeQuantity'],
       $interchange['UOM'],
       $interchange['ReferenceItem'],
       $interchange['InterchangeNotes'],
       $interchange['ItemEquivalentUOM'],
       $interchange['LanguageCode'],
       $interchange['SubBrandID'],
       $interchange['VMRSBrandID'],
       $interchange['QualityGradeLevel'],
       $interchange['InternalNotes']);
   $writer->writeSheetRow('Interchanges', $row);
  }
 }
 
 //-------------- digitalassets ----------
 $writer->writeSheetHeader('DigitalAssets',array('PartNumber'=>'string','FileName'=>'string','AssetType'=>'string','AssetID'=>'string','FileType'=>'string','Representation'=>'string','FileSize'=>'integer','FileHash'=>'string','Resolution'=>'string','ColorMode'=>'string','Background'=>'string','OrientationView'=>'string','AssetHeight'=>'integer','AssetWidth'=>'integer','UOM'=>'string','FilePath'=>'string','AssetDate'=>'string','AssetDateType'=>'string','Country'=>'string','LanguageCode'=>'string','URI'=>'string','Duration'=>'string','DurationUOM'=>'string','Frame'=>'integer','TotalFrames'=>'integer','Plane'=>'integer','Hemisphere'=>'string','Plunge'=>'integer','TotalPlanes'=>'integer','Description'=>'string','DescriptionCode'=>'string','DescriptionLanguageCode'=>'string'),array('widths'=>array(12,30,10,37,8,15,9,25,10,11,11,16,11,11,5,15,10,14,8,14,83,8,13,6,11,6,12,7,11,22,16,24),'freeze_rows'=>1, 'freeze_columns'=>1,['fill'=>'#ff0000'],['fill'=>'#ff0000'],['fill'=>'#ff0000'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#00ff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff']));
 foreach($items as $partnumber=>$item)
 {
...
<html>
    ...
    <body>
        ...
        <div class="container-fluid padding my-container">
			...
			<!-- Header -->
                        <h3 class="card-header text-start">Convert PIES (8.0) xml to Excel spreadsheet</h3>
...
if($streamXLSX)
{   
 $filename='Rhubarb_8_0_'.date('Y-m-d').'.xlsx';
