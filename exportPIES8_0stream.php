...
//   if($package['shippingheight']>0 && $package['shippingwidth']>0 && $package['shippinglength']>0 && $package['merchandisingheight']>0 && $package['merchandisingwidth']>0 && $package['merchandisinglength']>0)
...
//     $logs->logSystemEvent('Debug', $userid , 'Export of asset ['.$digitalassetrecord['assetid'].'] included (tagmatch on:'.$firstmatchedtagtext);
     ...
     $digitalasset['FileHash']=$digitalassetrecord['fileHashMD5'];
...
{// validated xml is ready to give to the user
 if($streamXML)
 {// download to file
  $filename='PIES_8_0_'.$documenttitle.'_FULL_'.date('Y-m-d').'.xml';
