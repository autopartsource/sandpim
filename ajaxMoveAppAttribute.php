...
{// bail out if this is a clinet we don't like
 $logs = new logs;
 $logs->logSystemEvent('accesscontrol',0, 'ajaxMoveAppAttribute.php - access denied to host '.$_SERVER['REMOTE_ADDR']);
 exit;
}
...
if(isset($_SESSION['userid']) && isset($_GET['id']) && isset($_GET['appid']))
{
 ...
 if($attribute=$pim->getAppAttribute($attributeid))
 {
  ...
  {
   $pim->incAppAttributeSequence($appid, $attributeid);
   ...
   $pim->logAppEvent($appid,$userid, $niceattribute.'] up-sequenced','');
   $result['success']=true;
  }
 }
