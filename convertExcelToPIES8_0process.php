...
include_once('./class/PIES8_0GeneratorClass.php');
  ...
  {     
   ...
   if(in_array('Items',$sheetNames) && in_array('Header',$sheetNames))
   {
    ...
    if(true) //isset($headerSheet[0][0]) && isset($headerSheet[0][0]) && isset($headerSheet[0][1]) && $headerSheet[0][1]=='7.1')
    {
     ...
     if(array_key_exists('TechnicalContact' ,$headerElementsList) && $headerElementsList['TechnicalContact']!='' && array_key_exists('ContactEmail' ,$headerElementsList) && $headerElementsList['ContactEmail']!='' && array_key_exists('PCdbPublicationDate' ,$headerElementsList) && $headerElementsList['PCdbPublicationDate']!='')
     {
     }
     else
     {
      $validUpload=false; 
      $inputFileLog[]='Header is missing TechnicalContact, ContactEmail or PCdbPublicationDate';
      $logs->logSystemEvent('rhubarb', 0, 'Header is missing TechnicalContact, ContactEmail or PCdbPublicationDate');
     }
    
     if(isset($itemsSheet[0][0]) && $itemsSheet[0][0]=='PartNumber' && isset($itemsSheet[0][1]) && $itemsSheet[0][1]=='PartTerminologyID' && isset($itemsSheet[0][2]) && $itemsSheet[0][2]=='BrandID')
     {
     }
     else
     {
      $validUpload=false;
      $inputFileLog[]='First row of Items sheet must start with these three columns: PartNumber, PartTerminologyID, BrandID';
      $logs->logSystemEvent('rhubarb', 0, 'First row of Items sheet must start with these three columns: PartNumber, PartTerminologyID, BrandID');
     }
    }
    else
    {
...
if($validUpload)
{
 foreach($headerSheet as $fields)
 {
  if(count($fields)>=2)
  {
   ...
   if($fields[0]=='ParentID' && trim($fields[1]!='')){$header['ParentID']=trim($fields[1]);}
   ...
   if($fields[0]=='PAdbPublicationDate' && trim($fields[1]!='')){$header['PAdbVersionDate']=trim($fields[1]);}
   if($fields[0]=='PCdbPublicationDate' && trim($fields[1]!='')){$header['PCdbVersionDate']=trim($fields[1]);}
  }
 }
...
//----------------------- marketing copy -----------------------
 ...
 for($i=0; $i<=count($itemsSheet[0])-1; $i++)
 {
  ...
  if($itemsSheet[0][$i]=='ProductCategoryCode'){$ProductCategoryCodeFieldIndex=$i;}
 }
 ...
 foreach($itemsSheet as $fields)
 {
  ...
  if($BrandIDfieldIndex >=0 && trim($fields[$BrandIDfieldIndex])!=''){$item['BrandID']=trim($fields[$BrandIDfieldIndex]);}
  ...
  if($ProductCategoryCodeFieldIndex >=0 && trim($fields[$ProductCategoryCodeFieldIndex])!=''){$item['ProductCategoryCode']=trim($fields[$ProductCategoryCodeFieldIndex]);}  
...
//    if(!isset($validEXPIcodes[$expi['EXPICode']][$expi['EXPIValue']]))
 ...
 if(in_array('Interchanges',$sheetNames))
 {
  ...
  $PartNumberFieldIndex=-1; $CompetitorPartNumberFieldIndex=-1; $ReferenceItemFieldIndex=-1; $InterchangeQuantityFieldIndex=-1; $UOMFieldIndex=-1; $InterchangeNotesFieldIndex=-1; $BrandIDFieldIndex=-1; $BrandLabelFieldIndex=-1; $SubBrandIDFieldIndex=-1; $SubBrandLabelFieldIndex=-1; $VMRSBrandIDFieldIndex=-1; $ItemEquivalentUOMFieldIndex=-1; $QualityGradeLevelFieldIndex=-1; $InternalNotesFieldIndex=-1; $LanguageCodeFieldIndex=-1;
  ...
  { // identify the named columns' IDs
   ...
   if($interchangesSheet[0][$i]=='BrandID'){$BrandIDFieldIndex=$i;}
   if($interchangesSheet[0][$i]=='BrandLabel'){$BrandLabelFieldIndex=$i;}
   if($interchangesSheet[0][$i]=='SubBrandID'){$SubBrandIDFieldIndex=$i;}
  ...
  if($PartNumberFieldIndex==0)
  {
   foreach($interchangesSheet as $fields)
   {
    ...
    if($BrandIDFieldIndex>=0 && trim($fields[$BrandIDFieldIndex])!=''){$interchange['BrandID']=trim($fields[$BrandIDFieldIndex]);}
    if($BrandLabelFieldIndex>=0 && trim($fields[$BrandLabelFieldIndex])!=''){$interchange['BrandLabel']=trim($fields[$BrandLabelFieldIndex]);}
    if($SubBrandIDFieldIndex>=0 && trim($fields[$SubBrandIDFieldIndex])!=''){$interchange['SubBrandID']=trim($fields[$SubBrandIDFieldIndex]);}
 ...
 if(in_array('DigitalAssets',$sheetNames))
 {
  ...
  { // identify the named columns' IDs
   ...
   if($digitalAssetsSheet[0][$i]=='FileHash'){$FileHashFieldIndex=$i;}   
  ...
  if($PartNumberFieldIndex==0)
  {
   foreach($digitalAssetsSheet as $fields)
   {
    ...
    if($FileHashFieldIndex>=0 && trim($fields[$FileHashFieldIndex])!=''){$asset['FileHash']=trim($fields[$FileHashFieldIndex]);}
 ...
 if(!$newdoc->schemaValidate('PIES_8_0_XSDSchema_Rev3_10_2_2025.xsd'))
 {
...
{
$filename='PIES_8_0_FULL_'.date('Y-m-d').'.xml';
