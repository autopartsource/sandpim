...
include_once('./class/ACES5_0GeneratorClass.php');
...
{// bail out if this is a clinet we don't like
 $logs = new logs;
 $logs->logSystemEvent('accesscontrol',0, 'exportACES50createJob.php - access denied to host '.$_SERVER['REMOTE_ADDR']);
 exit;
}    
...
$clientfilename='ACES_5_0_'.$keyedprofile['DocumentTitle'].'_'.$randomstring.'_FULL_'.date('Y-m-d').'.xml';
$localfilename=__DIR__.'/ACESexports/'.$randomstring;
$token=$pim->createBackgroundjob('ACES50xmlExport','started',$_SESSION['userid'],'',$localfilename,'ExportPurpose:'.$exportpurpose.';ExportType:'.$exporttype.';UpdateReceiverAppStates:'.$updatereceiverappstates.';receiverprofile:'.$receiverprofileid.';DocumentTitle:'.$keyedprofile['DocumentTitle'].';',date('Y-m-d H:i:s'),'text/xml',$clientfilename);
$logs->logSystemEvent('Export', $_SESSION['userid'], 'ACES 5.0 file ['.$clientfilename.'] export setup for houskeeper; apps:'.$appscount.' by:'.$_SERVER['REMOTE_ADDR']);
