...
$existinglocks=$pim->getLocksByType('RHUBARB71XLSX');
if(count($existinglocks))
{
 $logs->logSystemEvent('rhubarb', 0, 'Background (7.1 to Excel) found lock record (id:'.$existinglocks[0]['id'].') and declined to run');
 exit; 
}

$mylockid=$pim->addLock('RHUBARB', 'pid:'. getmypid());

$jobs=$pim->getBackgroundjobs('RHUBARB71XLSX','started');
...
if(count($jobs))
{
 ...
 $validupload=false;
 $xlsxdata='';
 $outputfilename=$jobs[0]['outputfile'];
 ...
 $pcdb = new pcdb($parameters['pcdbdatabasename']);
 $originalfilename= $inputputfilename=$jobs[0]['inputfile']; if(array_key_exists('originalfilename', $parameters)){$originalfilename=$parameters['originalfilename'];} 
...
//$validPartTypes=array(); $partTypes=$pcdb->getPartTypes('%'); foreach($partTypes as $partType){$validPartTypes[$partType['id']]=$partType['name'];}
 ...
 {
  $logs->logSystemEvent('rhubarb', 0, 'schema validation failed on '.$originalfilename.'. Schema errors:'.implode('. ',$schemaresults));
 }
 ...
 */
  ...
  //-------------- packages ---------
  $writer->writeSheetHeader('Packages', array('PartNumber'=>'string', 'PackageUOM'=>'string', 'QuantityofEaches'=>'integer', 'InnerQuantity'=>'integer',
     'InnerQuantityUOM'=>'string', 'Weight'=>'0.00', 'WeightsUOM'=>'string', 'PackageLevelGTIN'=>'string',
     'PackageBarCodeCharacters'=>'string', 'ShippingHeight'=>'0.00', 'ShippingWidth'=>'0.00', 'ShippingLength'=>'0.00', 'DimensionsUOM'=>'string', 'DimensionalWeight'=>'0.00',
     'WeightVariance'=>'0.00', 'StackingFactor'=>'0.00', 'ElectronicProductCode'=>'string', 'MerchandisingHeight'=>'0.00',
     'MerchandisingWidth'=>'0.00', 'MerchandisingLength'=>'0.00', 'ShippingScope'=>'string', 'Bulk'=>'string', 'RegulatingCountry'=>'string', 'TransportMethod'=>'string',
     'Regulated'=>'string', 'Description'=>'string', 'HazardousMaterialCodeQualifier'=>'string', 'HazardousMaterialDescription'=>'string',
     'HazardousMaterialLabelCode'=>'string', 'ShippingName'=>'string', 'UNNAIDCode'=>'string', 'HazardousPlacardNotation'=>'string', 'OuterPackageLabel'=>'string', 'TextMessage'=>'string', 'RegulationsExemptionCode'=>'string', 'PackingGroupCode'=>'string', 'WHMISFreeText'=>'string',
     'WHMISCode'=>'string', 'LanguageCode'=>'string'), array('widths'=>array(13,13,16,12,17,7,13,17,25,14,14,14,16,18,15,14,29,19,19,19,14,5,17,15,10,15,28,27,26,14,13,24,18,13,25,18,31,12,14),'freeze_rows'=>1, 'freeze_columns'=>1,['fill'=>'#ff0000'],['fill'=>'#ff0000'],['fill'=>'#ff0000'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#ffff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#00ff00'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],             ['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff'],['fill'=>'#0000ff']));
  foreach($items as $partnumber=>$item)
  {
  ...
  foreach($items as $partnumber=>$item)
  {
   ...
   {
    $row=array($partnumber,$digitalasset['FileName'],$digitalasset['AssetType'],$digitalasset['AssetID'],$digitalasset['FileType'],$digitalasset['Representation'],$digitalasset['FileSize'],$digitalasset['Resolution'],$digitalasset['ColorMode'],$digitalasset['Background'],$digitalasset['OrientationView'],$digitalasset['AssetHeight'],$digitalasset['AssetWidth'],$digitalasset['UOM'],$digitalasset['FilePath'],$digitalasset['AssetDate'],$digitalasset['AssetDateType'],$digitalasset['Country'],$digitalasset['LanguageCode'],$digitalasset['URI'],$digitalasset['Duration'],$digitalasset['DurationUOM'],$digitalasset['Frame'],$digitalasset['TotalFrames'],$digitalasset['Plane'],$digitalasset['Hemisphere'],$digitalasset['Plunge'],$digitalasset['TotalPlanes'],     $digitalasset['Description'],$digitalasset['DescriptionCode'],     $digitalasset['DescriptionLanguageCode']);    $writer->writeSheetRow('DigitalAssets', $row);
   }
  }
 ...
 $writeresult=false;
 if($fh=fopen($outputfilename, 'w'))
 {
  $writeresult=fwrite($fh, $xlsxdata);
  fclose($fh);
 }
 
 unlink($inputputfilename);
 $runtime=time()-$starttime;
 if($writeresult)
 {
  $pim->updateBackgroundjobDone($jobid,'complete',date('Y-m-d H:i:s'));
  $logs->logSystemEvent('rhubarb', 0, 'Processing of PIES file['.$originalfilename.'] completed in '.$runtime.' seconds'); 
 }
 else
